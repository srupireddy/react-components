...
export default class HomeLoanPurpose extends BaseComponent {
    ...
    render() {
        let radioItems = HomeLoanPurpose.options.map(
            (option) => (
                <DecorateWithImageAndLabel key={option.label} imageStyle={option.imageStyle} label={option.label} checked={this.state.selectedPurpose === option.value} containerStyle="col-md-2 col-xs-3 col-sm-3 radio-col">
